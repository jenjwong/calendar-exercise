...
const _genData = () => (
    EVENTS.map(({id, title, description, start, hours, dayDelta, color}) => {
        const dateOnDay = new Date(Date.now() + (dayDelta * MILLISECONDS_DAY));
        const year = dateOnDay.getFullYear();
        const month = dateOnDay.getMonth() + 1;
        const dayOfMonth = dateOnDay.getDate();
        const startDateTime = Date.parse(`${year}/${month}/${dayOfMonth} ${start}`);
